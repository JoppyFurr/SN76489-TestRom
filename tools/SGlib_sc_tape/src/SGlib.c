...
#define SG_write_to_VDPRegister(VDPReg,value)  do{ VDPControlPort=(value);     VDPControlPort=(VDPReg)|0x80;    }while(0)
#define SG_set_address_VRAM(address)           do{ VDPControlPort=LO(address); VDPControlPort=HI(address)|0x40; }while(0)
#else
...
/* JoppyFurr CHANGE:
 * With no interrupts in tape mode, the ISR's job is done
 * here. We poll the VDP status register to detect the interrupt. */
void SG_waitForVBlank (void) {

  volatile unsigned char VDPStatus;

  do {
      VDPStatus = VDPStatusPort;
